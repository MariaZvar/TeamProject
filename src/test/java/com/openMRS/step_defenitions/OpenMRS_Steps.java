...
import com.openMRS.pages.OpenMRS_AdvancedManagement;
...
public class OpenMRS_Steps {
    ...
    OpenMRS_AdvancedManagement manageUser = new OpenMRS_AdvancedManagement(driver);
    ...
    public void user_goes_to_the_system_administrations_and_clicks_advanced_administrations() {
        manageUser.clickAdministrations();
    }
    @Then("user clicks manage-person and finds the deleted person by {string}")
    public void user_clicks_manage_person_and_finds_the_deleted_person_by(String name, io.cucumber.datatable.DataTable dataTable) {
        manageUser.findDeletedPerson(name);
    }
    @Then("user changes the {string} and saves person")
    public void user_changes_the_and_saves_person(String address, io.cucumber.datatable.DataTable dataTable) {
        manageUser.changeAndSave(address);

    }
    @Then("user comes back and clicks Find Person Record")
    public void user_comes_back_and_clicks_find_person_record() {
        manageUser.findPersonRecord();
    }
    @Then("user finds updated person by {string}")
    public void user_finds_updated_person_by(String name, io.cucumber.datatable.DataTable dataTable) {
        manageUser.findUpdatedPerson(name);

    }
    @Then("user checks the {string}")
    public void user_checks_the(String updatedInfo, io.cucumber.datatable.DataTable dataTable) {
        manageUser.checkUpdatedInfo(updatedInfo);
